# services.py
import uuid
from datetime import date as DateObject  # Ensure datetime and timedelta
...
class WorkflowService:
    ...
    async def get_workflow_instance_with_tasks(
    ...
    async def create_workflow_instance(
        ...
        definition = await self.definition_repo.get_workflow_definition_by_id(
            instance_data.workflow_definition_id
        )
        if not definition:
            return None
        ...
        new_instance_pydantic = WorkflowInstance(
            ...
            or definition.name,  # Use instance_data.name, fallback to def.name
            user_id=instance_data.user_id,  # Must be provided
            status=instance_data.status
            or WorkflowStatus.pending,  # Default to pending if not provided
        ...
        created_instance = await self.instance_repo.create_workflow_instance(
            new_instance_pydantic
        )
        if not created_instance:
            return None

        for task_def in definition.task_definitions:
            task_due_datetime: datetime | None = None
    ...
    async def list_workflow_definitions(
        ...
        return await self.definition_repo.list_workflow_definitions(
            name=name, definition_id=definition_id
        )

    async def complete_task(self, task_id: str, user_id: str) -> TaskInstance | None:
        ...
        workflow_instance = await self.instance_repo.get_workflow_instance_by_id(
        ...
        if updated_task:
            ...
            if workflow_details:
                all_tasks_completed = all(
                    ...
                    for t in workflow_details.tasks
                )
                if all_tasks_completed:
                    workflow_instance.status = models.WorkflowStatus.completed
                    await self.instance_repo.update_workflow_instance(
                        workflow_instance.id, workflow_instance
                    )
        return updated_task
    ...
    async def list_instances_for_user(
        ...
        return await self.instance_repo.list_workflow_instances_by_user(
            user_id,
    ...
    async def create_new_definition(
        self,
        name: str,
        ...
        definition = WorkflowDefinition(
            name=name, description=description, task_definitions=task_definitions
        )
    ...
    async def update_definition(
        ...
        return await self.definition_repo.update_workflow_definition(
            definition_id, name, description, task_definitions
        )

    async def delete_definition(self, definition_id: str) -> None:
        from repository import DefinitionInUseError, DefinitionNotFoundError
    ...
    async def archive_workflow_instance(
        ...
        updated_instance = await self.instance_repo.update_workflow_instance(
            instance.id, instance
        )
        return updated_instance

    async def unarchive_workflow_instance(
        self, instance_id: str, user_id: str
    ) -> WorkflowInstance | None:
        instance = await self.instance_repo.get_workflow_instance_by_id(instance_id)
    ...
    async def generate_shareable_link(
        ...
        await self.instance_repo.update_workflow_instance(
            instance_id, updated_instance_pydantic
        )
    ...
    async def get_workflow_instance_by_share_token(
        ...
        instance = await self.instance_repo.get_workflow_instance_by_share_token(
            share_token
        )
