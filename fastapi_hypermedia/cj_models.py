...
class ItemData(BaseModel):
    name: str
    value: StrictBool | int | float | dict | list | None | datetime.datetime | datetime.date | str = PydanticField(None,
                                                                                                                  description="Value of the data item")
    prompt: str | None = PydanticField(None, description="Human Readable prompt for the data")
    type: str | None = PydanticField(None, description="Type of the data")
    input_type: str | None = PydanticField(None,
                                              description="Suggested input type (e.g., 'text', 'checkbox', 'number', 'select')")
    render_hint: str | None = PydanticField(None,
                                               description="A hint for how to render the data item (e.g., 'textarea', 'colorpicker')")


class QueryData(ItemData):
    options: list[str] | None = PydanticField(None, description="List of options for 'select' input type")
...
class Template(BaseModel):
    ...
    href: str | None = None
    method: str | None = PydanticField("POST", description="HTTP method for the template")
    prompt: str | None = None
    rel: str | None = None
...
class CollectionJson(BaseModel):
    ...
    template: list[Template] | None = PydanticField(None, description="Templates for the collection")
